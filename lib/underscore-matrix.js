...
_matrix.sSummary = function(s,a){
    if(isNaN(s)){
        return {error:"first arg must be numeric"};
    } else {
        s = parseFloat(s);
    };
    var ch = _matrix.isSquareMatrix(a);
...
_matrix.inverse = function(a){
    ...
    var detA = _matrix.det(a).value;
    if(detA == 0){
        return {error:"Matrix determinant = 0"};
    }
    var adj = [];
    for(var i=0; i< a.length; i++){
        adj[i] = []
        for(var j=0; j< a.length; j++){
            adj[i][j] = Math.pow(-1,(i+j))*_matrix.det(_matrix.minor(j,i,a).value).value;
        }
    };
    res.value = _matrix.smul((1/detA), adj).value;
    res.error = false;
    return res;
};
